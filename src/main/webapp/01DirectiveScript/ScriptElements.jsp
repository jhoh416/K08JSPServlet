...
<!-- 
선언부 : 스크립트렛이나 표현식에서 사용할 메서드를
	선언할때 사용된다. 선언부에서 선언된 메서드는 해당 JSP가
	서블릿(Java파일)으로 변환될떄 멤버메서드 형태로 선언된다.
 -->
<%!
...
<body>
<!-- 
스크립트렛 : 주로 JSP코드를 작성하고 실행할 떄 사용한다.
	스트립트렛은  body, head 어디서든 사용할 수 있다. 또한 <script>
	<style> 태그 내부에서도 사용할 수 있다.
	여기에 기술된 코드는 _jspService()메서드 내에 포함된다.
	그러므로 해당 영역에는 메서드를 선언할 수 없다.
표현식 : 변수를 웹브라우저상에 출력할 때 사용한다.
	JS의 document.write()와 동일한 역할을 한다. 표현식 사용시
	주의할 점은 문장끝에 ;(세미콜론)을 붙이지 않는다. -->
<%
