...
<body>
	...
	<!-- 지시어를 통한 인클루드는 포함할 파일의 소스를 그대로 현재
	문서에 포함시킨 후 컴파일을 진행한다. 따라서 하나의 페이지라
	생각하면 된다. 그러므로 newVar1은 정상적으로 출력된다.
	 -->
	<p>외부파일에 선언한 변수 : <%= newVar1 %></p>
	
	
	<h3>액션태그로 페이지 포함하기</h3>
	<!-- 액션태그를 이용한 인클루드는 문자열과 표현식 두가지 모두
	사용할 수 있다.
	 -->
	...
	지시어 
		...
		- 따라서 같은 페이지를 의미한다.
		- page 영역과 request영역이 공유 된다.
		- jsp 코드와 변수등이 포함된 경우 사용한다.
	액션태그
		...
		공유된다.
		- HTML태그만 있거나 단독으로 실행가능한 JSP코드만 있을때
		사용한다.
	 -->
