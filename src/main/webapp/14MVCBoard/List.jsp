...
<body>
    <h2>파일 첨부형 게시판 - 현재 페이지 : ${ map.pageNum } (전체 :)</h2>
    <!-- 검색폼 -->
    <form method="get">  
...
<c:choose>
	...
	<c:otherwise>
	<!-- 게시물이 있을 대 컬렉션에 저장된 목록의 갯수만큼 반복한다. -->
		<c:forEach items="${ boardLists }" var="row" varStatus="loop">
		<!-- 
		가상번호 계산하기
		=> 전체게시물갯수 - (((페이지번호-1) * 페이지당 게시물수) 
			+ 해당루프의 index)
		참고로 varStatus 속성의 index는 0부터 시작된다.
		
			전체게시물의 갯수가 5개라면.. 페이지당 2개씩만 출력한다면..
			1페이지의 첫번개 게시물 번호 : 5 - (((1-1) * 2) + 0) = 5
			2페이지의 첫번개 게시물 번호 : 5 - (((2-1) * 2) + 0) = 3
		 -->
		<tr align="center">
            <td>
...
<a href="../mvcboard/view.do?idx=${ row.idx }">${ row.title }</a> 
            ...
            적용되어 있지 않다. 따라서 첨부파일이 있을때만 다운로드 링크를 출력한다. -->
