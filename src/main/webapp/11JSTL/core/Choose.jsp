...
<!-- 
choose 태그
	: 하위에 when, otherwise태그를 가지고 있다. 여러 조건이
	있는 경우 사용하기에 적합하다.
 -->
	<!-- 변수선언 -->
	...
	<!-- form태그에 속성이 없으면 get방식으로 현재 페이지로 폼값이
	전송된다. -->
	<form>
	...
	<c:if test="${ not (empty param.kor or empty param or empty param.math) }">
		<!-- 3과목의 점수를 더한후 평균값을 구한다. 이 값은 변수 avg에 
		저장된다. -->
		<c:set var="avg" value="${ (param.kor + param.eng + param.math) / 3 }"></c:set>
		평균 점수는 ${avg }으로
		<!-- 평균점수를 통해 조건을 걸어 학점을 출력한다.
		when 태그는 if 혹은 else if와 동일하고 otherwise는 else와 동일하다.
		 -->
		<c:choose>
