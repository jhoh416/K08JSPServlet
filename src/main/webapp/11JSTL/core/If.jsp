...
<!DOCTYPE html><html>
<head><meta charset="UTF-8"><title>JSTL - if</title></head>
<body>
<!-- 
...
속성정리
 ...
 -->
	...
	받은 후 tjouen/1234 인 경우에는 'tjoeun님 하이롱'dlfkrh cnffurgksek.
...
<!-- 	<form method="get">
		...
		<br/>
		<li>user : ${ param.user }</li>
		<input type="submit" value="로그인"/>
		${ user }
	<c:if test="${ user eq 'tjoeun' }" var="result9">
		${ user }님 하이롱. <br/>
	</c:if>
	...
	<c:if test="${ not empty param.user }">
		...
		변수에 저장한다.
		if태그는 else구문이 따로 없으므로 not을 통해 두번재 if문을
		구성해야 한다.
		 -->
