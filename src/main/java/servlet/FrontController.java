...
public class FrontController extends HttpServlet{
	...
	해야 한다. 만약 서블릿에 이 메서드가 정의되지 않았다면 405에러가
	발생하게 된다. 즉 해당방식의 요청을 처리할 수 없다는 의미이다.
	 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		//reauest내장객체를 통해 현재 요청된 URL을 얻어온다. 즉 웹브라우저의
		//주소표시줄에 있는 전체경로명에서 HOST(localhost)를 제외한 나머지
		//경로를 얻어올 수 있다.
		String uri = req.getRequestURI();
		//URL에서 마지막 /의 index를 얻어온다.
		int lastSlash = uri.lastIndexOf("/");
		//앞에서 얻은 index를 통해 URL을 자른다. 즉 마지막에 있는 요청명만
		//남긴다.
